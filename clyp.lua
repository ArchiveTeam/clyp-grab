...
find_item = function(url)
  ...
  for pattern, name in pairs({
    ["^https?://clyp%.it/([a-z0-9]+)$"]="clip",
    ["^https?://clyp%.it/user/([a-z0-9]+)$"]="user",
    ["^https?://(s3%.amazonaws%.com/.+)$"]="asset",
    ["^https?://(static%.clyp%.it/.+)$"]="asset",
...
allowed = function(url, parenturl)
  ...
  for pattern, type_ in pairs({
    ["^https?://clyp%.it/([a-z0-9]+)$"]="clip",
    ["^https?://clyp%.it/user/([a-z0-9]+)$"]="user",
    ["^https?://www%.clyp%.it/([a-z0-9]+)$"]="clip",
    ["^https?://www%.clyp%.it/user/([a-z0-9]+)$"]="user",
    ["^https?://(s3%.amazonaws%.com/.+)$"]="asset",
    ["^https?://(static%.clyp%.it/.+)$"]="asset",
    ["^https?://([^%./]+%.cloudfront%.net/.+)$"]="asset"
  }) do
  ...
  if not string.match(url, "^https?://[^/]*clyp%.it") then
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  local function check(newurl)
    ...
    url_ = string.gsub(url_, "^(https?://[^/:]+):?[0-9]*(/+)$", "%1%2")
  ...
  if allowed(url)
    and status_code < 300
    and item_type ~= "asset"
    and not string.match(url, "^https?://audio%.clyp%.it/")
    and not string.match(url, "^https?://a%.clyp%.it/")
    and not string.match(url, "^https?://soundwave%.clyp%.it/") then
    html = read_file(file)
    ...
    if string.match(url, "^https?://clyp%.it/user/[a-z0-9]+$") then
      check("https://api.clyp.it/User/" .. item_value .. "/Uploads")
      check(url .. "/followers")
      check("https://api.clyp.it/user/" .. item_value .. "/followerrelationships")
      check(url .. "/following")
      check("https://api.clyp.it/user/" .. item_value .. "/followingrelationships")
    end
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["clyp-qxs20mqgp1uwtbjj"] = discovered_items,
    ["urls-e8ufh8awah6ki99e"] = discovered_outlinks
  }) do
